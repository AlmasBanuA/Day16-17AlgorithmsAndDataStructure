 ...
 * Logic -> To Merge Sort an array, we divide it into two halves, sort the two halves
   independently, and then merge the results to sort the full array. To sort a[low, high),
   we use the following recursive strategy:
 b.Base case: If the array length is 0 or 1, it is already sorted.
 c.Reduction step: Otherwise, compute mid = low + (high - low) / 2, recursively sort the
   two arrays a[low, mid) and a[mid, high), and merge them to produce a sorted
   result.
 */
...
public class MergeSort {
	...
	/**
	 * creating method mergeSort to Sort of list of Strings.
	 * 
	 * @param array
	 * @param low
	 * @param high
